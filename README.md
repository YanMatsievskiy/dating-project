Веб-платформа для знакомств (Django/DRF)

01. Описание
Этот проект представляет собой веб-платформу для знакомств, реализованную на Django и Django REST Framework (DRF).
...
- Обнаружение взаимных лайков (матчей) и возможность обмена контактами.
- API документирован с помощью Swagger.
- Проект упакован в Docker-контейнеры и запускается с помощью docker-compose.
- Используется PostgreSQL для хранения данных и JWT для аутентификации.
...
03. Запуск с помощью Docker Compose
Для запуска проекта необходим Docker и Docker Compose.

04. Установка Docker и Docker Compose
Следуйте официальным инструкциям:
- [Docker](https://docs.docker.com/get-docker/)
- [Docker Compose](https://docs.docker.com/compose/install/)

05. Запуск приложения:
1.  **Откройте терминал** в папке проекта (где лежит `docker-compose.yml`).
2.  **(Опционально) Создайте файл `.env` для настройки переменных окружения:**
    ```bash
    # .env
...
06. Просмотр в браузере:
1.  Откройте веб-браузер.
2.  Перейдите по адресу `http://localhost:8000/api/docs/` для просмотра интерактивной документации Swagger.
3.  API-эндпоинты будут доступны по адресу `http://localhost:8000/api/...`.

07. Дополнительные команды:
